...
open import Data.System using (System; _≤_; _≈_; Systems[_,_]; ≤-refl; ≤-trans; discrete; Systems-MC; Systems-SMC)
open import Data.Values monoid using (module ≋; module Object; Values; ≋-isEquiv; module Algebra)
...
opaque
  ...
  map : (Fin A → Fin B) → System (Values A) (Algebra.Valuesₘ A) → System (Values B) (Algebra.Valuesₘ B)
...
opaque
  unfolding map
  open System
  map-≤ : (f : Fin A → Fin B) {X Y : System (Values A) (Algebra.Valuesₘ A)} → X ≤ Y → map f X ≤ map f Y
  ⇒S (map-≤ f x≤y) = ⇒S x≤y
...
opaque
  ...
  map-≤-refl
      : (f : Fin A → Fin B)
      → {X : System (Values A) (Algebra.Valuesₘ A)}
      → map-≤ f (≤-refl {x = X}) ≈ ≤-refl
...
opaque
  ...
  map-≤-trans
      : (f : Fin A → Fin B)
      → {X Y Z : System (Values A) (Algebra.Valuesₘ A)}
      → {h : X ≤ Y}
      → {g : Y ≤ Z}
...
opaque
  ...
  map-≈
      : (f : Fin A → Fin B)
      → {X Y : System (Values A) (Algebra.Valuesₘ A)}
      → {g h : X ≤ Y}
      → h ≈ g
      → map-≤ f h ≈ map-≤ f g
  map-≈ f h≈g = h≈g

Sys₁ : (Fin A → Fin B) → Functor Systems[ Values A , Algebra.Valuesₘ A ] Systems[ Values B , Algebra.Valuesₘ B ]
Sys₁ {A} {B} f = record
    { F₀ = map f
    ; F₁ = λ C≤D → map-≤ f C≤D
    ; identity = map-≤-refl f
...
opaque
  unfolding map
  map-id-≤ : (X : System (Values A) (Algebra.Valuesₘ A)) → map id X ≤ X
  map-id-≤ X .⇒S = Id (S X)
...
opaque
  unfolding map
  map-id-≥ : (X : System (Values A) (Algebra.Valuesₘ A)) → X ≤ map id X
  map-id-≥ X .⇒S = Id (S X)
...
Sys-homo {A} f g = niHelper record
  ...
  where
    ...
    module _ (X : System (Values A) (Algebra.Valuesₘ A)) where
      opaque
        unfolding map-∘-≤ map-∘-≥
...
module _ {f g : Fin A → Fin B} (f≗g : f ≗ g) (X : System (Values A) (Algebra.Valuesₘ A)) where
...
module NatCat where
  ...
  Sys .F₀ n = Systems[ Values n , Algebra.Valuesₘ n ]
  Sys .F₁ = Sys₁
...
module NatMC where
  ...
  module _ (f : Fin A → Fin B) where
    ...
    module A = System-⊗ (Values A) (Algebra.Valuesₘ A)
    module B = System-⊗ (Values B) (Algebra.Valuesₘ B)

    open CommutativeMonoid⇒ (Push.₁ f)
    open Morphism Systems[ Values B , Algebra.Valuesₘ B ] using (_≅_)

    opaque
      unfolding map
      ε-≅ : discrete (Values B) (Algebra.Valuesₘ B) ≅ map f (discrete (Values A) (Algebra.Valuesₘ A))
      ε-≅ = record
    ...
    private
      ...
      F : Functor Systems[ Values A , Algebra.Valuesₘ A ] Systems[ Values B , Algebra.Valuesₘ B ]
      F = Sys₁ f
    ...
    opaque
      ...
      unitaryʳ
          ...
          ∘′ ⊗-homo-≃.⇒.η (X , discrete (Values A) (Algebra.Valuesₘ A))
          ∘′ B-MC.id B.⊗₁ ε-≅.from
          B-MC.≈ B-MC.unitorʳ.from
      unitaryʳ {X} = Setoid.refl (S X)

    Sys-MC₁ : StrongMonoidalFunctor (Systems-MC (Values A) (Algebra.Valuesₘ A)) (Systems-MC (Values B) (Algebra.Valuesₘ B))
    Sys-MC₁ = record
        { F = Sys₁ f
  ...
  opaque
    ...
    Sys-MC-identity : MonoidalNaturalIsomorphism (Sys-MC₁ id) (idF-StrongMonoidal (Systems-MC (Values A) (Algebra.Valuesₘ A)))
  ...
  Sys .F₀ n = Systems-MC (Values n) (Algebra.Valuesₘ n)
  Sys .F₁ = Sys-MC₁
...
module NatSMC where
  ...
  module _ (f : Fin A → Fin B) where
    ...
    private
      ...
      F-MF : StrongMonoidalFunctor (Systems-MC (Values A) (Algebra.Valuesₘ A)) (Systems-MC (Values B) (Algebra.Valuesₘ B))
      F-MF = NatMC.Sys.₁ f
    ...
    Sys-SMC₁ : SymmetricMonoidalFunctor (Systems-SMC (Values A) (Algebra.Valuesₘ A)) (Systems-SMC (Values B) (Algebra.Valuesₘ B))
    Sys-SMC₁ = record
        { F-MF
  ...
  Sys-SMC-identity : SymmetricMonoidalNaturalIsomorphism (Sys-SMC₁ id) (idF-StrongSymmetricMonoidal (Systems-SMC (Values A) (Algebra.Valuesₘ A)))
  ...
  Sys : Functor Nat SymMonCat
  Sys .F₀ n = Systems-SMC (Values n) (Algebra.Valuesₘ n)
  Sys .F₁ = Sys-SMC₁
